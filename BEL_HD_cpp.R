...
#'@import picasso
#'@import Rcpp
BEL_HD = function(X,Y,Oiter = 300,Iiter = 100, Oburnin = 150,Iburnin = 20,thres_s=0.01){
  ptm <- proc.time()
  p = ncol(X)
  n = length(Y)
  # Initialization
  ...
  beta = picasso(X, Y, nlambda=1000, type.gaussian="naive",lambda.min.ratio = 0.01)$beta[,1000]+0.00001
  ...
  Tau_sum = sum(1/rinvgauss(rep(1, p_new),sqrt(gam_1^2/beta[index_act]^2),gam_1^2))# squared
  V_sum = sum(1/rinvgauss(rep(1, p_new),sqrt(gam_2^2/lambda[index_act]^2),gam_2^2))
  for(iter in 2:Oiter){
    ...
    if(p<3200){
      grad = grad(X,Y,Lambda[iter,],Beta[iter,])
...
Inner_loop = function(X,Y,index_act,beta_int,lambda_int,gamma_1,gamma_2,Iiter=100,Iburnin=20){
  ...
  for (iter_i in 2:Iiter) {
    ...
    Tau_I[iter_i,] = 1/rinvgauss(rep(1, p_new),sqrt(gamma_1^2/Beta_I[iter_i-1,]^2),gamma_1^2) # squared
    V_I[iter_i,] = 1/rinvgauss(rep(1, p_new),sqrt(gamma_2^2/Lambda_I[iter_i-1,]^2),gamma_2^2) # squared
