...
err = matrix(0,10,10)
cor_mtx = matrix(0,10,10)
wro_mtx = matrix(0,10,10)

for(fold in 1:10){
  set.seed(fold)
  Valid_index = sample(train_ind,10)
  Train_index = subset(train_ind, !(train_ind %in% Valid_index))
  # this part added
  thres = seq(0.01,0.1,0.01)
  for(ii in 1:10){
    set.seed(ii)
    res5 = BEL_HD(X[Train_index,], Y[Train_index],thres_s=thres[ii],Quick=FALSE)
    cor_5= sum(abs(res5$beta[c(1:15)])>0)
    wro_5 = sum(abs(res5$beta[-c(1:15)])>0)
    mspe_5 = mean((Y[Valid_index] - X[Valid_index,]%*%res5$beta)^2)
    err[fold,ii] = mspe_5
    cor_mtx[fold,ii] = cor_5
    wro_mtx[fold,ii] = wro_5
...
Recall = A/10
Prec = A/(A+B)
F1 = 2*Recall*Prec/(Prec+Recall) # choose threshold 0.08 for BEL_HD
set.seed(87489)
for (ii in 1:100) {
  sigma = matrix(rep(0.1,p_rand*p_rand),p_rand)+diag(0.9,p_rand)
  X_rand = rmvnorm(n=n, mean=rep(0,p_rand), sigma=sigma)
...
write.csv(res_1, file = "Result_crime_r4.csv")
