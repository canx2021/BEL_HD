...
for (ii in 1:100) {
  ...
  b[1:bb] =  rnorm(bb,5,3)
  error_term = rt(n,3)
  y = x %*% b + (rbinom(n,1,.5)*2-1)*3 + error_term/sd(error_term)
  ...
  cor_1= sum(abs(beta_SCAD[c(2:(bb+1))])>0)
  wro_1 = sum(abs(beta_SCAD[-c(1:(bb+1))])>0)
  ...
  cor_2= sum(abs(beta_la[c(1:bb)])>0)
  wro_2 = sum(abs(beta_la[-c(1:bb)])>0)
  ...
  cor_3= sum(abs(res3$est.bic[c(1:bb)])>0)
  wro_3 = sum(abs(res3$est.bic[-c(1:bb)])>0)
  ...
  cor_4= sum(abs(b_est_sis2[c(1:bb)])>0)
  wro_4 = sum(abs(b_est_sis2[-c(1:bb)])>0)
  
  # BEL_HD
  res5 = BEL_HD(X,Y,thres_s =0.01,Quick=FALSE)
  
  MSE5 = sum((res5$beta - b)^2)/p
  cor_5= sum(abs(res5$beta[c(1:bb)])>0)
  wro_5 = sum(abs(res5$beta[-c(1:bb)])>0)
  
  #PICASSO
  res6 = picasso(X, Y, nlambda=1000, type.gaussian="naive",lambda.min.ratio = 0.01)$beta[,1000]
  MSE6 = sum((res6 - b)^2)/p
  cor_6= sum(abs(res6[c(1:bb)])>0)
  wro_6 = sum(abs(res6[-c(1:bb)])>0)
  ...
  cor_7= sum(abs(est.LS$beta.MAP[c(1:bb)])>0)
  wro_7 = sum(abs(est.LS$beta.MAP[-c(1:bb)])>0)
...
write.csv(res_1, file = "Result_p10_100_800.csv")
